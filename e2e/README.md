...
2026-08-03 在闲机器上（load 5.05、无 `xcodebuild`）拿到两组硬证据：

| 跑法 | 退出码 | 结果 |
|---|---|---|
| 原样 8 条 | **0** | `✔ e2e 全过`；**123 步全 COMPLETED、42 条断言、零 FAILED** |
| 塞进一条必然失败的 flow | **1** | `✘ 09-deliberate-fail.yaml 没过` + `✘ e2e 有没过的`，**其余 8 条照绿** |

第二组才是真正值钱的那一组，它一次证了三件事：

1. **单条红传得到套件退出码**——`ww_maestro` 的 `PIPESTATUS[0]` 真接住了；
2. **零耦合噪声**——只红被弄坏的那一条，不像忙机器上那样「红的每轮换一批」；
3. **第三道闸分得清「flow 真红」和「App 被掀了」**——真红时它没有误报「整轮作废」。

⛔ **只有第一组（全绿）是不够的。** `assertNotVisible` 在页面根本没渲染出来时
会凭空通过，而这一套里两条「不许有英文」的 flow 恰恰都是这个形状。
**绿比红更可疑，所以每次动过 `run.sh` / `lib.sh` 都要把第二组重跑一遍**
——塞一条断言写死的假 flow，跑完删掉，一分钟的事。

## ⛔ 挑档位的测试别挑默认档

`08-batch-exact` 初稿挑的是 **108**——一串念珠的数，最该验的那个数。

它是错的。`TemplateCatalog.defaultBatchStep` 就是 108，
所以把视图里 `vm.setBatchStep(batchStep)` 整行删掉（选中的档位彻底不生效），
这条 flow **照样绿**：默认值恰好等于我挑的那个数。
它验的从来不是「选中的那一档生效了」。

改挑 **500**（与 108 无倍数关系）之后拿同一个变异咬：

```
✘ Assert that "本次 501" is visible... FAILED     退出码 1
```

截图取证，屏幕上是 **109**——正是 `1 + 默认的 108`，
**也正是初稿断言的那个数**。同一份坏代码，初稿绿、改稿红。

**通用形状**：测「用户选的那个值生效了吗」，就不能选中那个碰巧等于默认值的选项。
同族的还有布尔开关只验一个方向（值与默认相同时，串键和不落盘都照样绿）。
