...
enum TemplateCatalog {
    ...
    /// 颜色候选。用户在这里选的是**图形色**——进度环的弧和那枚图标。
    ///
    /// **同一个色值要同时站在浅色底和深色底上**：`ProgressRing` 收的是一个固定
    /// hex，不随 `colorScheme` 走。要两边都够得着 1.4.11 的 3:1，相对亮度只能落在
    /// `L ∈ [0.1215, 0.2771]` 这条窄带里，两头的极值色一支都用不了。
    ///
    /// 原先第三支是晨曦金 `#D9B679`（L=0.496），浅色底上只有 **1.798:1**。
    /// 它是这么溜进来的：`Palette` 里那句「只作光晕、不承载文字」的纪律，
    /// 连同为它专设的守卫 `晨曦金不承载文字`，把的都是**文字**那扇门；
    /// 而它是从**图形色**这扇门进来的，那扇门后面当时根本没有审计。
    /// 用户点中它，圆环就淡到看不出——他不会来反馈「对比度只有 1.8」，
    /// 他只会更少地打开今日页。
    ///
    /// 现已换成带内的暖金 `#8A6D3B`（浅 4.53 / 深 3.79），
    /// 并补上了那扇缺的门：`模板色板每支在两个背景上都够得着图形对比度`。
    static let colorChoices = [
        Palette.Light.fulfilled, Palette.Light.accent, "#8A6D3B",
