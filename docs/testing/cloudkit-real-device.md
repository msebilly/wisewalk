...
### RD-04 — §5.6 到达顺序风险（fresh/cleared B）
...
只对 RD-04 已用证据捕获的 snapshot-half 或 relation-nil 窗口执行：

1. 在窗口内终止 App，依次做重新启动、锁屏、解锁；记录每一步 UI/日志/实体计数。
2. 全程不得崩溃、不得显示假圆满/假无课、不得错误归项。
3. 窗口结束后必须逐笔自愈到 A 的精确集合、目标与账本。
