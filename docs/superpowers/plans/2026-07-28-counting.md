...
struct TimerView: View {
    ...
    var body: some View {
        ...
        .onReceive(tick) { now in
            vm.refresh(at: now)
            // 心跳每 10 秒落一次盘。崩溃后能损失的时长上限就是这个间隔——
            // 而恢复时用的是 updatedAt 而非「现在」，所以 App 崩在三天前
            // 也不会给用户记上 72 小时的打坐。
...
回到前台立刻 refresh，不等下一次 tick；心跳每 10 秒落盘一次，崩溃能损失
的时长上限就是这个间隔。
...
- [ ] **Step 10: 给 `TemplateCatalog` 补图标与颜色候选**
...
并在 `Tests/TemplateCatalogTests.swift` 末尾追加：
...
final class RecoveryCoordinator {
    ...
    func accept(_ item: PendingRecovery, at now: Date = Date(),
