...
| 音效 / 震动的开关界面 | 第 5 卷 | §6.2 要求两者「各自可独立开关」，`AppSettings.soundEnabled` / `hapticEnabled` 本卷已就位、默认全开、被 `CounterView` 读取，但**全卷没有一个 `Toggle`**——能力有了，用户够不着。与下面一条同批，一并在设置页落地 |
