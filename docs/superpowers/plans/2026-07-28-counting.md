...
private func makeTimer(goal: Int? = 1800) throws
    -> (TimerViewModel, DraftStore, DayLedger, PracticeItem) {
    let ctx = ModelContext(try ModelContainerFactory.inMemory())
    let ledger = DayLedger(context: ctx, deviceName: "iPhone·TEST")
    let drafts = DraftStore(context: ctx, ledger: ledger)
    let items = PracticeItemStore(context: ctx)
    let item = try items.create(name: "打坐", measureType: .duration, unit: "",
                                ...
                                colorHex: Palette.Light.fulfilled)
    return (TimerViewModel(item: item, drafts: drafts, ledger: ledger), drafts, ledger, item)
}
...
@Test func 计时中一笔流水都不写() throws {
    let (vm, _, ledger, item) = try makeTimer()
    let start = 北京(7, 28, 6, 0)
    try vm.start(at: start, timeZone: 北京时间)
    for m in 1...10 { vm.refresh(at: 北京(7, 28, 6, m)) }
    #expect(try ledger.sessions(on: 20260728, itemID: item.id).isEmpty)
}

@MainActor
@Test func 心跳按间隔打不是每次refresh都写盘() throws {
    let (vm, drafts, _, item) = try makeTimer()
    let start = 北京(7, 28, 6, 0)
    try vm.start(at: start, timeZone: 北京时间)
...
@Test func 结束时按秒记一笔() throws {
    let (vm, drafts, ledger, item) = try makeTimer()
    let start = 北京(7, 28, 6, 0)
...
@Test func 不足一秒就结束不留空账() throws {
    ...
    let start = 北京(7, 28, 6, 0)
    try vm.start(at: start, timeZone: 北京时间)
    let s = try vm.finish(at: start.addingTimeInterval(0.4), timeZone: 北京时间)
    #expect(s == nil)
    #expect(try ledger.sessions(on: 20260728, itemID: item.id).isEmpty)
    #expect(try drafts.pendingDrafts().isEmpty)
}
...
@Test func 重复结束不会记两笔() throws {
    ...
    let start = 北京(7, 28, 6, 0)
    try vm.start(at: start, timeZone: 北京时间)
    try vm.finish(at: 北京(7, 28, 6, 10), timeZone: 北京时间)
    let second = try vm.finish(at: 北京(7, 28, 6, 20), timeZone: 北京时间)
    #expect(second == nil)
...
@Test func 今日总时长把先前坐过的算进来() throws {
    ...
    _ = try vm.finish(at: 北京(7, 28, 20, 10), timeZone: 北京时间)
...
@Test func 未开始时刷新与心跳都是空操作() throws {
    let (vm, drafts, _, _) = try makeTimer()
...
Run: `make test`
Expected: 全绿，总数增至 192

- [ ] **Step 9: 变异验证「不靠滴答累加」真的被约束住**

把 `refresh(at:)` 临时改成 `elapsed += 1`（并把 `startedAt` 的 guard 保留），跑 `make test`。
Expected: `时长按时间戳差值算而不是靠滴答累加`、`结束时按秒记一笔`、`时钟文本随走时更新` 一起变红。
**改回去**，再跑一次确认恢复全绿。
...
struct TimerView: View {
    ...
    var body: some View {
        ...
        // 回到前台立刻补齐挂起期间的时长，不用等下一次 tick。
        .onChange(of: scenePhase) { _, phase in if phase == .active { vm.refresh() } }
        .onDisappear { commit() }
        .alert("出了点问题", isPresented: .constant(failure != nil)) {
            Button("知道了") { failure = nil }
        } message: { Text(failure ?? "") }
