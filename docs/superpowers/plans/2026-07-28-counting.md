...
>
> **这个变异第一次跑出来红了六条**，因为当时 `record` 那句直接读 `self.amount`——
> 清空一挪到前面，`record` 当场收到 0，凡是靠它写盘的测试全塌。
> 那不是「清早了」被抓住，是**变异同时改了两件事**（顺序 + 载荷），
> 证不出任何东西。实现里已把校验过的数抄成局部量 `let amount = self.amount`，
> 变异这才只剩顺序这一个变量。**跑出六条红说明代码有耦合，不说明测试有效。**
>
...
是因为本任务 Step 1 的 `模板色板每支在两个背景上都够得着图形对比度` 就要读 `colorChoices`。
...
Run: `make test`
Expected: 全绿，总数增至 250

- [ ] **Step 14: 变异验证「共用上下文」这条守卫会咬人**

把 `AppEnvironment` 里 `self.drafts = DraftStore(context: ctx, ledger: ledger)` 临时改成
`self.drafts = DraftStore(context: ModelContext(container), ledger: ledger)`，跑 `make test`。

Expected: **只有** `装配后三个仓储共用同一个上下文` 变红，且红在最后那句
「commit 的流水没落盘」上。

变异挑 `drafts` 而不是 `items`，是因为 `items` 那条路上每一步都自己 save，
换个 context 也照样落盘、照样查得到——**那个变异照不出任何东西**。
`commit` 是全仓库唯一一处「insert 在 A、save 在 B」的地方，只有它对共用与否敏感。

⚠️ **若实测红的条数或位置与这里写的不符，照实报，别改测试去凑。**
尤其留意有没有变成崩溃或抛错——那种「红」证明不了共用与否。
...
## Task 13.5: 快照追加——把同步迟到的定课补回当日应做

**Files:**
- Modify: `Sources/Store/DayLedger.swift`（`plan` 加两个必填参数 + 私有 `appendLateArrivals`）
- Modify: `Sources/Features/Today/TodayViewModel.swift`（传参 + 改掉已过期的注释）
- Modify: `Sources/Features/ManualEntry/ManualEntryViewModel.swift`（传参 + 改掉已过期的注释）
- Test: `Tests/DayPlanAppendTests.swift`（新建）
- Test: 现有各处 `ledger.plan(` 调用补两个实参

**这一任是 2026-07-30 用户对 `docs/design-spec.md` §5.6 写侧的定案：走「追加快照 + createdAt 判据」。**

§5.6 原本列的第一条路（等 CloudKit 初次同步完成再允许定格）没走，理由记在 spec 里：
Core Data 有 `NSPersistentCloudKitContainer.eventChangedNotification`，
**SwiftData 在 iOS 17 上没把它暴露出来**，能不能拿到这个信号要等第 3 卷接真容器才知道。
不能把一条可能不存在的信号写进地基。

**为什么不推到第 3 卷再做**：这套逻辑是纯的，现在就测得了；
而第 3 卷是调同步的时候，那时最分不清「逻辑错了」和「同步没到」。
别在那个当口还要顺手发明新语义。
...
```
grep -h '@Test' Tests/*.swift | grep func | sed 's/.*func //' | sort | uniq -d
```

Expected: 无输出。

创建 `Tests/DayPlanAppendTests.swift`：
...
Run: `make test`
Expected: 编译不过——`plan` 还是两个参数的旧签名。这一步就是要它编不过。

- [ ] **Step 3: 改 `DayLedger.plan` 的签名并加追加逻辑**

`Sources/Store/DayLedger.swift`，在原有文档注释末尾**追加**以下两段，
然后换掉函数签名与 `existingPlan` 那一支：
...
函数体其余部分一字不动。随后在 `plan` 与 `merge` 之间插入：
...
同一文件 `reload` 的文档注释里这一段已经**不再成立**，整段换掉：
...
`Sources/Features/ManualEntry/ManualEntryViewModel.swift`：
...
同一处那段带 ⚠️ 的「第 3 卷之前必须先答 §5.6」已经答了，换成：
...
- [ ] **Step 5: 把现有测试里所有 `ledger.plan(` 调用补上两个实参**

逐处补 `dayStartHour:` 与 `timeZone:`。**照抄各处原本算 dayKey 用的那把尺子**：
测试若用 `DayKey.today(dayStartHour: 3, ...)` 算的 key，这里就传 3；
用字面量 `20260728` 的传 0，时区传该测试原本用的那个（多为北京时间）。
**不要图省事一律传 0 加 `.current`**——`.current` 在本机是 PDT，本卷已两次因此假绿。

- [ ] **Step 6: 跑测试确认通过**

Run: `make test`
Expected: 全绿，总数增至 258

- [ ] **Step 7: 变异验证四条**

一次只改一处，跑完 `git restore` 还原再改下一处。**串行，不许并发**。

| # | 变异 | Expected |
|---|---|---|
| 1 | `return born < dayKey` 改成 `return true` | 只红 `今天新立的定课不会被追加进今天` |
| 2 | `guard !late.isEmpty else { return existing }` 整句删掉 | 红 4 条（下有实测记录） |
| 3 | `guard !item.isArchived, ...` 去掉 `!item.isArchived` | 只红 `已归档的迟到定课不会被追加` |
| 4 | 把 `born < dayKey` 的判据也加进初次定格那一支 | 目标守卫会红，但会连坐一大片并崩溃（下有实测记录） |

⚠️ **实测与此处不符就照实报，别改测试去凑。**
⚠️ **跑之前确认没有别的 agent 在树里**（`pgrep -xl xcodebuild`）——
2026-07-30 出过两个 reviewer 同树跑变异、互相打成 SIGKILL 的事故，一批结论作废。

**实测记录（2026-07-30，我原先的预测有两条写错了，此处按实测改正）：**

变异 1、3 与预测一致，各只红一条。

**变异 2 实测红 4 条**，多出 `读取计划不改写任何源快照`（`FulfillmentTests.swift:182`）
与 `重新载入不会重复生成快照`（`TodayViewModelTests.swift:313`）。
两条都是「数快照条数」的断言——删掉 `guard` 后每次对已有快照调 `plan()` 都插一条空快照，
凡是数数量的测试都会红。**抓得比预测更宽，不是假红**（无崩溃，`Test run with 258` 汇总正常）。
我写「只红两条」时想当然了。

**变异 4 的预测彻底写错。** 目标守卫 `初次定格仍旧收下全部在册定课` 确实红了、有效，
但它连坐了十五条以上并触发 **12 次 `Index out of range` 崩溃**，
swift-testing 因进程崩溃从未打出 `Test run with 258` 汇总。

根因：库里既有测试的定课多是运行期 `PracticeItem(...)` 造的（`createdAt` = 真实的今天），
却冻结到硬编码的过去日（如 `20260728`）。判据一旦套进初次定格支，
这些项 `born` 不小于 `dayKey` 便被全数剔除，`required` 变空，
测试里那些 `rows[0]` 之类的下标就越界了。
**我写这条预测时隐含假设「跑测当天 ≤ 那些硬编码日期」，这个前提在真实跑测日不成立。**

崩溃在**测试代码**，不在产品代码——`grep -rnE '\w+\[[0-9]+\]|\.first!|\.last!' Sources/`
是空的，产品代码一个数组下标都没有，空的应做集合不会让它崩。
`空快照的一天在定课同步进来后能自愈` 正是拿空集合跑的，绿。

这条变异因此**只能证明守卫会红，不能证明它精确**。留着它是因为「守卫会红」本身要证；
但读结果时别把连坐那一片当成守卫的覆盖面。

### Task 13.5 续：判据换成 `activatedAt`

**Step 1–7 落地后（`b2853d8`）复查发现的行为变更，2026-07-30 用户拍板补此段。**

`PracticeItemStore.activeItems()` 按 `isArchived == false` 过滤，所以归档的课
**根本不会传进 `plan()`**。于是：某门课在定格时是归档状态 → 不在快照里 →
用户下午点「恢复」（`PracticeItemStore.unarchive`，Task 18 配了按钮）→
`!isArchived` 成立、不在 `known` 里、`createdAt` 又是很久以前 → **被追加进今天**。

后果：今天只有念佛，做完了显示圆满；下午恢复了打坐，圆满退回未圆满——
**把用户已经挣到的圆满收回去**。Step 1–7 之前不会这样：已有快照就一个字都不加。

难处在于**同一条代码路径同时服务着一个正确的场景**：昨天在 iPad 上恢复了打坐，
今天 iPhone 才同步到——那时打坐确实是今天该做的，追加是对的。
光看 `isArchived` 分不出这两者，差别只在**恢复发生在哪一天**。

`createdAt` 回答的是「这门课什么时候立的」，而这里真正要问的是
「**今天开始的时候它活没活着**」。立课那天之后被归档又恢复，这两个问题就分家了。

- [ ] **Step 8: 写失败测试**

追加进 `Tests/DayPlanAppendTests.swift` 末尾：
...
- [ ] **Step 9: 给 `PracticeItem` 加 `activatedAt`**

`Sources/Models/PracticeItem.swift`，在 `updatedAt` 之后插入：
...
- [ ] **Step 9.5: 过 CloudKit 白名单那道闸**

**这一步是我（写计划的人）漏掉的，由实现者撞出来。**
`Tests/CloudKitConstraintTests.swift` 的 `allowed` 是一份**白名单**，
任何新增建模属性都会让它变红。注释写明这是**故意的**：

> 改用白名单：任何新增的建模属性都会让本测试变红，逼迫开发者停下来确认——
> 这不是缓存/聚合量吗？确认无误后，再把它加进下面对应实体的白名单。

所以这不是「顺手把测试改绿」，是**这道闸要求做的那个判断**。判断记录如下：

**`activatedAt` 不是缓存，也不是可变聚合量，可以进同步 schema。**

- **不是缓存**：它不由任何别的字段推导得出，记的是一件真实发生过的事——
  这门课哪一刻成为活跃状态。跟已在白名单里的 `createdAt` / `updatedAt` 同族。
- **不是聚合量**：不是任何东西的和、计数或跑动总数。闸真正要拦的是
  「今日总数」这类会跟流水打架的数——`activatedAt` 跟数量毫无关系。
- **LWW 冲突下不会编数**：两台设备各恢复一次，最后胜出的无论哪个，
  都是一个**真的发生过**的恢复时刻。不像跑动总数那样会合出一个谁都没做过的数。
- **改不动已定格的过去**：快照一旦写下就不再回溯改写，
  所以 `activatedAt` 事后变了也动不了任何历史日的判定，只影响将来的追加。
- **兜底方向是保守那一侧**：CloudKit 推来的老记录没有这个字段时取模型默认值
  `.distantPast`，算出的 dayKey 小于任何真实日期，于是一律追加——
  进了分母顶多显示未圆满，**漏掉才是替他免单**。

登记进白名单（`Tests/CloudKitConstraintTests.swift` 的 `PracticeItem` 那一项）：
...
⚠️ 这个 `]` 和外层的 `]` 都别弄丢——2026-07-30 实现者就是在这一处被 `edit`
吃掉两个右括号、留下编译不过的树。**改完立刻 `view` 复核。**

- [ ] **Step 10: `unarchive` 一并更新，判据改用 `activatedAt`**

`Sources/Store/PracticeItemStore.swift`：
...
`Sources/Store/DayLedger.swift` 的 `appendLateArrivals`，把这两段文档换掉：
...
判据本身两处 `item.createdAt` 改成 `item.activatedAt`：
...
Run: `make test`
Expected: 全绿，总数增至 260

- [ ] **Step 12: 变异验证**

串行，一次一处，跑完 `git restore` 还原。基线必须先提交。

| # | 变异 | 必须红 |
|---|---|---|
| 1 | `unarchive` 里删掉 `item.activatedAt = now` | `今天恢复的归档定课不会被追加进今天` |
| 2 | 判据两处 `activatedAt` 改回 `createdAt` | 同上 |
| 3 | `self.activatedAt = activatedAt ?? createdAt` 改成 `?? Date()` | `迟到的定课会被追加进当日应做` |

⚠️ **「必须红」是下限，不是全集。** Step 7 的实测记录里，我对变异波及面的两次预测
都偏乐观（一次少算 2 条，一次整条写错）。**把实际红的全部照抄回来**，
多出来的逐条说明是「抓得更宽」还是「连坐/崩溃」——后者证明不了守卫的精度。

**实测记录（2026-07-30）：**

| # | 实际红 | 判读 |
|---|---|---|
| 1 | 只红 `今天恢复的归档定课不会被追加进今天` | 与预测一致 |
| 2 | 只红 `今天恢复的归档定课不会被追加进今天` | 与预测一致 |
| 3 | 红 4 条：`迟到的定课会被追加进当日应做`、`追加过一次就不会再追加`、`追加项的目标取自追加时那条快照`、`空快照的一天在定课同步进来后能自愈` | **抓得更宽，非连坐**：四条都在 `DayPlanAppendTests` 里，形状全是「该追加的没追加」。这些测试都用过去的 `createdAt` 且不显式传 `activatedAt`，兜底一改成 `Date()`，整条追加路径就对它们全体失效。无崩溃，260 汇总正常 |

⚠️ **认账标准再加一条：`Test run with` 出现还不够，后面那个数字也得对得上。**
变异 3 第一次跑出的是 `Test run with 177 tests`（且有一条红重复出现两次）——
邻居的 xcodebuild 在我开跑之后才起来，抢了同一个模拟器。
干净重跑是 260 / 4 issues / 零崩溃。**残缺的跑要整轮作废重来，不许从里面挑结论。**

### Step 13–17：双审收口（2026-07-30，两审均只读、纯 A 堆证据）

Task 13.5 是本卷唯一一个「计划由我当天临时写、未经审查就直接实现」的任务，
且它往 CloudKit 同步 schema 里加了字段。补做规范符合性 + 代码质量两审，
硬规矩：全程只读，不许改文件、不许跑 `make test`、不许跑变异
（上一次两个 reviewer 同树跑变异互相 SIGKILL，一大批结论作废）。

**逐字节比对结论**：16 个 swift 块全部命中，块 0–6 在 `b2853d8`、块 7–15 在 `61a2e9b`。
**实现者没有代码块级偏离，错在计划本身。**

#### Step 13：`unarchive` 补幂等守卫

加 `activatedAt` 之前，对已经活着的项多调一次 `unarchive` 是彻底的空操作；
加了之后它会把 `activatedAt` 顶到此刻，于是真正同步迟到的项从此判成
「今天才激活」、再也补不回当天——**方向是「多」**。UI 上一个按钮连点两下、
或列表刷新时重放一次动作就够触发。

这正是刚写进纪律第 9 条第 ⑨ 项的形状：**新增守卫只证明新行为对，证明不了旧行为没被动。**
Task 13.5 一任之内犯了两次（第一次是 `createdAt` 版改掉了「已有快照就一个字都不加」）。
...
#### Step 14：删掉一条编出来的规矩，把不对称老实写清

`grep -rn '从明天算起' docs/ Sources/ Tests/` 的每一处出现，源头都是 Task 13.5 自己的文字。
**设计规范里从来没有「今天新立的课从明天算起」这条规矩，是我编的**，
用来给「初次定格收今天新立的课、追加不收」这个不对称作辩护。

初次定格：`activeItems.filter { !$0.isArchived }`，**照收今天刚立的**。
追加：`activeSince < dayKey`，**把今天才激活的挡在外面**。
于是「今天新立的课今天算不算数」取决于该日有没有别的设备先定过格。

用户 2026-07-30 拍板走候选一（维持现状，写清不对称），不选：
- 候选二「改问：当日第一条快照定格那一刻它活着吗」——两条路会自动一致，
  且不再需要 `dayStartHour`/`timeZone` 两个参数，但跨设备比时刻受时钟偏差影响。
- 候选三「再加 `archivedAt`」——同步 schema 又多一个字段，且仍答不了多轮归档恢复。

`born` 改名 `activeSince`：它装的是「最近一次激活日」不是「出生日」，
却在整个功能最要害的那一行上让人读成后者。
...
#### Step 15：测试网眼（两处，都能让守卫为错误的原因通过）

1. **「另开上下文」是死代码，而且建错了东西。**
   原写法 `let 独立 = ModelContext(try ModelContainerFactory.inMemory())` 建完就 `_ =` 扔掉，
   断言查的还是原 `ctx`；更糟的是 `inMemory()` 造的是**另一个容器**，就算用了也永远查 0。
   仓库正确写法是 `ModelContext(ctx.container)`（`DayLedgerTests.swift:307`、
   `DraftStoreTests.swift:201/209`、`AppEnvironmentTests.swift:39` 三处都是）。
   后果：`没有迟到项时一条快照都不多写` 与 `追加过一次就不会再追加` 都分不清
   「已 save」和「只 insert 没 save」。

2. **三条测试不钉前置的定格步骤。** 若定格悄悄产出空快照，`known` 为空，
   念佛会作为「迟到项」被追加路径重新加回来，末尾断言照样成立——
   测不出「往非空快照追加」与「从空快照整体重建」的区别，而名字承诺的正是前者。

3. `今天恢复的归档定课不会被追加进今天` 没断言恢复真成功：
   删掉 `item.isArchived = false`，它会被 `guard !item.isArchived` 挡掉，**测试为错误的原因通过**。
...
**真验落盘**（`没有迟到项时一条快照都不多写` 与 `追加过一次就不会再追加` 两处同改）：
...
**钉住前置定格 + 断言恢复真成功**（以 `今天恢复的归档定课不会被追加进今天` 为例，
`昨天恢复…` 与 `空快照的一天…` 同理）：
...
完整文件见磁盘 `Tests/DayPlanAppendTests.swift`（11 条）。本段只列改动处，避免重复计数。

#### Step 16：陈旧注释与 spec（纪律第 ⑧ 条，第三次栽）

五处注释已成谎：`plan` 说「尚无快照时才落库」、`appendLateArrivals` 说「把 `createdAt`
也换算」、说「昨天定格空了就永远是空的」（补记页会补）、`TodayViewModel` 说
「已有快照时不会再写」、`ManualEntryViewModel` 说「没有才建、有了不动」。
`design-spec.md` §5.6 三处仍写 `createdAt`，`:161` 字段清单漏了 `activatedAt`。

**Step 9.5 写的过闸理由第 4 条是错的**——原文「快照写下不再回溯改写，故它事后变了也动不了
历史判定」。写下的那条快照本身确实不改写，但补记页会对历史日调 `plan()`，
**合并出来的历史判定确实会变**。正确说法：改的是「新增一条」，不是「改写已有的那条」；
历史日的判定会因此变化，这是有意的，因为补记页上的日子恰恰还能补做。

#### Step 17：变异验证（串行，基线 `699996d` 已提交）

| 变异 | 预测 | 实测 |
|---|---|---|
| M1 删掉 `unarchive` 的 `guard item.isArchived` | 只红新增那条幂等守卫 | ✅ `Test run with 261 tests`，1 issue，只红 `对本来就活着的课调恢复不会顶掉它的激活时刻` |
| M2 删掉 `appendLateArrivals` 的 `try saveOrRollback()` | 红 `追加过一次就不会再追加` | ✅ 261 tests，1 issue，正是它 |
| **反证**：审查前的测试文件 + 同一个 M2 | 应当**全绿**，证明那个洞真存在 | ✅ `Test run with 260 tests ... passed` |

**反证这一条是本次审查价值的硬证据**：同一个「把追加的快照不落盘」的错误，
旧网眼一条都照不出，补上 `ModelContext(ctx.container)` 之后当场咬人。
...
⚠️ **初稿这两条是空壳 `{ /* 注释 */ }`——空测试就是假绿，本卷最严重的规划失误之一。**
实现者照注释里的 spec 填实并**钉住了前提**（`sessions.count == 4` 那句），
它才分得清「roundCount 算错了」与「四笔流水压根没记进去」。下面是落地版：
...
### Step 12：收工（`96401a9` + `d810f99`）— 273 全绿

实现者报了 4 处与计划不符，**4 处全对**（本卷实现者累计驳回我 12 次，12 次都对）。

#### ① 计划自相矛盾：坐数算了却不显示

Step 4b 明写「`.duration` 项在时长之后追加「· N 坐」，N 为 0 时不显示」，
**而 Step 8 逐字节的 `TodayView` 块里没有这段**。实现者按铁规矩照 Step 8 落地、
停下来报告，处置正确——`roundCount` 算好了不显示，等于用户那条
「打坐按时间和坐数量两个数」的定案没交付。

补法：加在 `FulfillmentBadge.progressText` 这个**纯函数**里，不是加在视图里。
`TodayView` 有**两处**要说这句话——可见副标题和 VoiceOver 的 `accessibilityLabel`。
分开写迟早漏一处，**而漏掉的那处必定是听不见的那处**。
`rounds: Int = 0` 有默认值，六个既有调用点一字不动。
...
#### ② 空壳测试（见 Step 4b，已把计划里的 stub 换成落地版）

#### ③ Step 10 的 `git add` 清单漏了 `DayLedger.swift`

Step 4b 明明往 `DayLedger` 加了 `roundCount(...)`，提交清单却没列它——
照抄会提交出**编译不过的快照**。实现者把 6 个改动文件都 add 了，正确。

#### ④ Step 11 手动验证未执行

非交互模式操作不了模拟器 UI。**这一条对 Task 15–19 同样成立，别当它做过了。**

### Step 13：变异实测

计划为 Task 14 没有定义任何变异。实现者自拟 4 个（M1 坐数只数条数 / M2 撤销过滤反向 /
M3 去掉勾选类守卫 / M4 删掉结尾的 `reload`），全部咬人，实测与它自己的预测一致。
坐数显示补上后我又跑了两个：

| 变异 | 预测 | 实测 |
|---|---|---|
| 坐数 `rounds > 0` 改成 `>= 0`（0 坐也显示） | 红 | ✅ 273 tests，`进度文案区分计数与计时` 红 3 处 |
| **删掉 `accessibilityLabel` 里的 `rounds: row.roundCount`**（可见副标题保留） | **预期抓不住** | ⚠️ **273 全绿** |

**第二条是负结果，比正结果值钱**：VoiceOver 标签悄悄丢了坐数，一条测试都没红。

**⛔ 已知覆盖空洞（Task 15–19 全部适用）：视图层的接线没有任何测试。**
本卷测的是 ViewModel 与纯函数；`TodayView` 里把哪个字段传给哪个参数，
改错了不会红。目前唯一的对策是**结构性**的：把要说两遍的话收进一个纯函数，
让「会忘的地方」只剩一处——不是测试覆盖，别把它当成覆盖。
真要覆盖得引入 ViewInspector 或快照测试，本卷不做。
...
- [x] **Step 8: 审查收口——删掉三条冗余且用了 `.current` 的测试**

实现者按「逐字节照抄」交付了 7 条测试后**停下来报告**：`CounterViewTests.swift` 是全仓唯一用 `DayKey.today()`（内含 `Date()` + `TimeZone.current`）的测试文件，违反本项目的固定时区规矩。它没有自己改测试——处置正确。

但它把危害判成「顶多零点那一秒假红」，**判轻了**。复核发现两件事：

**其一，这是结构性假绿，不是假红。** `CounterViewModelTests.swift:19-31` 早就留了一段实验记录，说的正是这个走样。我把它真的注进去跑了一遍——`CounterViewModel.start` 里 `self.timeZone = timeZone` 改成 `.current`：

| | 结果 |
|---|---|
| `CounterViewModelTests`（固定北京时间） | **红 6 条 / 10 个 issue** |
| `CounterViewTests`（新增这 7 条） | **一条都不红** |

连 `页面退出时提交一笔` 里的 `total(on: DayKey.today()) == 108` 都照绿——断言与实现用的是同一把歪尺子。

**其二，这三条是冗余的，而且是更弱的那一份。**

| 这里新增（弱） | 仓库里早就有（强） |
|---|---|
| `进页面会承接未提交的草稿` | `重进计数器接着上次的数`——还查了「不该多出一份草稿」 |
| `页面退出时提交一笔` | `结束时写一笔流水并清掉草稿`——还查了 `startedAt`/`endedAt`，钉死 `20260728` |
| `一声没数就退出不留空记录`（按天过滤） | `一声没念就结束不留空账`——**`库里一条流水都没有` 全库不限天** |

原以为它们至少验了 `AppEnvironment` 的装配（用 `env.drafts`/`env.ledger` 而非手工拼），但 `AppEnvironmentTests.装配后三个仓储共用同一个上下文` 已经覆盖。**一分钱新东西都没买到。**

文件叫 `CounterViewTests`、测试叫「页面退出时提交一笔」，读的人会以为这一页被覆盖了——这就是 Task 13.5 双审点名过五次的「名不副实」。用户 2026-07-31 拍板：**删掉，并在文件里写清这一页真正测到了什么。**

Step 1 的代码块已改成删后的最终版（4 条测试 + 一段「测到了什么、没测到什么」的文件头注释）。**总数 280 → 277，全卷终值 339 → 336。**

- [x] **Step 9: 收工**

```bash
git add Tests/CounterViewTests.swift docs/superpowers/plans/2026-07-28-counting.md
```
...
> **跨零点那个 `if` 块里 `dayRounds` 也要跟 `committedTotal` 一起归零**——
> 只归零秒数、不归零坐数，收坐时得到的是「昨天的坐数 + 1」，
> 屏幕会说「今日 2 坐」而新的一天只坐了这一坐。方向是**「多」**。
> 秒数报多了用户未必看得出，「今日 2 坐」是一句他能读懂的假话。
> `roundCount` 由 Task 14 Step 4b 提供。
> 相应地把 `Tests/TimerViewModelTests.swift` 补上下面这一条，
> 并给既有的 `坐过零点后屏幕上的今日不带昨天的数` 补一行坐数断言
>（该测试原本只验秒数；打坐是「时间和坐数量两个数」，它只验了一个）：
>
> ```swift
...
> **⚠️ 本任务只做正计时。倒计时与引磬（§6.3.1，2026-07-31 定案）在 Task 16.5。**

> **⚠️ `TimerViewTests.swift` 里只许放真正的 View 测试。**
> 这份规格的初版写了 5 条，其中 **4 条是冒牌货**，是 Task 15 同一个毛病的复发：
>
> | 初版写的 | 病 | 仓库里早就有 |
> |---|---|---|
> | `挂起再回来时长按时间戳差值补齐` | ViewModel 测试住进 View 文件 | `时长按时间戳差值算而不是靠滴答累加` |
> | `结束写入的是秒数与计时来源` | ↑ 且用 `DayKey.today()` 跟一个也用 `.current` 的实现比 | `结束时按秒记一笔`（多钉 `startedAt`/`endedAt`/`isRunning`/`pendingDrafts`，钉死 `20260728`）|
> | `一秒未坐就退出不留空记录` | ↑ 同上 | `不足一秒就结束不留空账`（`库里一条流水都没有` 全库不限天）|
> | `走时文案分秒补两位` | **逐字抄** | `DurationFormatTests` 里 `clock(0)`/`clock(65)`/`clock(3665)` 三个值一模一样 |
>
> 全部删掉。**这一页真正能测的只有能否实例化**——视图接线（哪个字段传给哪个参数）
> 本卷没有任何覆盖，Task 14 与 Task 15 的 M3 变异都实测证明了。别为了凑数字造假绿。
...
Run: `make test`
Expected: 全绿，总数增至 279

- [ ] **Step 5: 提交**

```bash
git add Sources/Features/Timer/TimerView.swift Tests/TimerViewTests.swift \
        Sources/Features/Timer/TimerViewModel.swift Tests/TimerViewModelTests.swift
git commit -m "$(cat <<'EOF'
feat: 计时器页
...
- [x] **Step 7: 审查收口（2026-07-31 完成）**

实现者交付 `d97c5c9`，279 全绿、三个代码块逐字节零偏差。**它自己报了一处计划隐患，成立且已修**：

**跨零点时 `dayRounds` 会替用户多报一坐。** 计划原文只说「处理完跨零点归零之后 `dayRounds += 1`」，
而那个 `if` 块里只归零了 `committedTotal`。夜坐 23:40 起坐、次日 0:10 收坐，
`dayRounds` 拿到的是「昨天的坐数 + 1」。红绿实测：

```
RED   ✘ (vm.dayRounds → 2) == 1   at TimerViewModelTests.swift:252
GREEN ✔ Test run with 279 tests in 0 suites passed
```

修法与 `committedTotal` 同款：`if` 块里补 `dayRounds = 0`。归零的代价是新那天若已有别处记的
流水会漏一坐，方向是**「丢」**不是「多」，且下次 `start()` 一次补齐。
既有测试 `坐过零点后屏幕上的今日不带昨天的数` 补了一行坐数断言——它原本只验秒数，
**而打坐是「时间和坐数量两个数」，它只验了一个**。测试数不变（补的是断言不是测试）。

实现者的判断里有一处我不同意，记下来：它说这个 bug「基本不可见」，因为收坐即 `dismiss`。
但 `commitAndClear` 是 `finish()` 和 `record(seconds:)` 共用的，**四小时闸门弹窗后按「记上」走的是后者**，
那条路上用户会先看到副标题再离开。而且「不可见的假话」也还是假话——
按这个逻辑推下去，`committedTotal` 那次归零也不必做。

**变异实测（实现者跑的，三条预测全中）**：

| 变异 | 预测 | 实测 |
|---|---|---|
| 删 `dayRounds += 1` | 🔴 | 🔴 `(vm.dayRounds → 0) == 1` |
| `start()` 里 `dayRounds = rounds` → `= 0` | 🔴 | 🔴 `:389 (vm.dayRounds → 0) == 1` |
| 副标题 `vm.dayRounds` → `vm.committedTotal` | 🟢 | 🟢 **279 全绿** |

第三条**又一次坐实视图接线零覆盖**（Task 14、15 之后第三次）。副标题会把坐数显示成秒数，
是个一眼可见的真 bug，全卷没有一条测试会红。

- [ ] **Step 8: 收工**

```bash
git add Sources/Features/Timer/TimerViewModel.swift Tests/TimerViewModelTests.swift \
        docs/superpowers/plans/2026-07-28-counting.md
git commit -m "$(cat <<'EOF'
fix: 跨零点时坐数也要归零，否则屏幕替用户多报一坐

commitAndClear 里跨零点的那个 if 块原本只归零 committedTotal。夜坐 23:40 起坐、
次日 0:10 收坐，dayRounds 拿到的是「昨天的坐数 + 1」，副标题会说「今日 2 坐」，
而新的一天只坐了这一坐。

秒数报多了用户未必看得出，「今日 2 坐」是一句他能读懂的假话。方向是「多」，
正是 committedTotal 那次归零所避开的东西的反面。

归零的代价是新那天若已有别处记的流水会漏一坐，方向是「丢」不是「多」，
且下次 start() 一次就补齐——与 committedTotal 完全同款。

既有的「坐过零点后屏幕上的今日不带昨天的数」补一行坐数断言。它原本只验秒数，
而打坐是「时间和坐数量两个数」，它只验了一个。红绿都跑过：
补上断言先红 (vm.dayRounds → 2) == 1，加上归零才绿。

这处是实现者自己在交付报告里指出的，不是我查出来的。
...
> §6.3.1，用户 2026-07-31 四问定案。**Task 16 只做正计时，这里补倒计时。**

**Files:**
- Create: `Sources/Core/WAVWriter.swift`
- Create: `Sources/Core/QingSound.swift`
- Create: `Sources/Core/QingScheduler.swift`
- Modify: `Sources/Core/ClickSound.swift`（改用 `WAVWriter`）
- Modify: `Sources/Core/AppSettings.swift`（加 `qingEnabled`）
- Modify: `Sources/Features/Timer/TimerViewModel.swift`（倒计时封顶）
- Modify: `Sources/Features/Timer/TimerView.swift`（档位选择 + 到零响磬）
- Test: `Tests/QingSoundTests.swift`，并补 `Tests/TimerViewModelTests.swift`

### ⛔ 这个任务里唯一真正危险的一处

**封顶会毁掉四小时闸门赖以判断的证据。**

四小时闸门（`TimerViewModel.implausibleAfter`）存在的全部理由是分辨这两件事——它们在代码里长得一模一样，都是 `now − startedAt` 一个大数：

- 用户起坐、锁屏、把手机放在一边坐了半小时（**必须记**）
- 用户早课起坐忘了按结束，晚上才想起来打开 App（**绝不能记**）

一旦 `elapsed` 封顶在 1800，**坐了 30 分钟和忘了 8 小时读数一模一样**，闸门永远不会响，App 会安安静静给他记上 30 分钟。那是编数。
...
|---|---|---|
| `elapsed` | 显示、入账 | **封顶** |
| `rawElapsed` | **只给四小时闸门** | **不封顶** |

设了 30 分钟倒计时、8 小时后才回来 → 仍然拦住、仍然问用户实际坐了多久。

### ⛔ 第二处：到零之后必须停心跳

到零后走时不再往上爬，但心跳若照打，草稿的 `updatedAt` 会一直往前走。App 这时候崩了，`DraftRecovery.suggestedAmount` 按 `updatedAt − startedAt` 估，会给出**远超倒计时**的数——绕过封顶从后门把「多」放进来。

`heartbeatIfNeeded` 加 `guard !hasReachedZero`。停了之后 `updatedAt` 定格在到零那一刻，崩溃恢复估出来的正好约等于倒计时。

### 已知并接受的洞

**崩溃恢复不知道那一坐是倒计时。** `countdownSeconds` 只活在 ViewModel 内存里，不进草稿（`SessionDraft` 是本地表，加字段不贵，但没必要）。App 崩了重启，恢复出来的草稿按正计时处理。

不堵，因为**恢复路径本来就更安全**：`RecoveryCoordinator` 按心跳 `updatedAt` 估算并**弹窗问用户**，不会静默落账。挂起-恢复（不经过启动）那条路上 `countdownSeconds` 还在，封顶照常生效。

### ⛔ 第三处：提前下坐必须撤掉已排的通知

用户设 30 分钟、10 分钟就按了结束，20 分钟后手机突然响一声引磬——这是个必现的骚扰。`finish` / `record` / `abandon` **三个出口都要撤**。

且 `start()` 是幂等的、回前台会再调一次，所以通知请求必须用**固定标识符**，让后来的顶掉前一个；否则回前台五次就排了五声。

- [ ] **Step 1: 写失败测试**

创建 `Tests/QingSoundTests.swift`：
...
补进 `Tests/TimerViewModelTests.swift`：
...
Run: `make test`
Expected: 编译失败，`cannot find 'QingSound' in scope`

- [ ] **Step 3: 抽出 `WAVWriter`**

创建 `Sources/Core/WAVWriter.swift`：
...
把 `ClickSound.wavData()` 的后半段（从 `var data = Data()` 到 `return data`）换成：

```swift
        return WAVWriter.container(pcm: pcm, sampleRate: sampleRate)
```

**`ClickSoundTests` 一个字都不许改。** 它那六条正是这次重构的安全网——
逐字节比对头部的断言若变红，说明抽取过程中改动了行为。

- [ ] **Step 4: 写 `QingSound`**

创建 `Sources/Core/QingSound.swift`：
...
`finish` 里的闸门改成读 `rawElapsed`，**抛出去的那个数也要改**：
...
> **⛔ 这一处漏了不会有任何测试红。** 闸门测试若只写 `#expect(throws: TimerViewModelError.self)`，
> 「响了」和「响了但报的数是编的」它分不出来——所以上面那条测试**必须**写成
> `#expect(throws: TimerViewModelError.implausibleDuration(seconds: 28800))`。
> 这是「名字比断言宽」在闸门上的翻版：`倒计时不会让四小时闸门失灵` 里的
> 「失灵」本该管两件事，只写 `.self` 就只管了一件。

`clearRound()` 里补 `rawElapsed = 0`。

> **⚠️ 纪律第 6 条**：`countdownSeconds` 不要在 `start()` 里清。用户回前台会再走一遍
> `start()`，清掉就等于把他设的倒计时悄悄换成正计时——走时会突然开始往上爬。

- [ ] **Step 6: `AppSettings` 加引磬开关**

`Key` 里加 `static let qing = "settings.qingEnabled"`，构造函数里比照 `sound`/`haptic` 默认为 `true`。
另加 `static let qingLockScreenNoticed = "settings.qingLockScreenNoticed"`（**默认 false**，
不进构造函数那三行显式落盘）。两个属性：
...
并补一条 `Tests/AppSettingsTests.swift`——**新加设置必须同时加持久化测试**，
既有那条 `三个设置都能跨实例持久` 的注释里写着为什么，而它自己已经名不副实
（现在有五项），一并改名为 `音效震动与一日起始都能跨实例持久`：
...
- [ ] **Step 7: 写 `QingScheduler`**

创建 `Sources/Core/QingScheduler.swift`：
...
未开始时显示档位选择（`TimerViewModel.countdownChoices` + 「不限时」+ 自定义，
自定义走已有的时长选择器）。开始时：
...
前台到零时响一次（用 `onChange(of: vm.hasReachedZero)`，只在 `false → true` 那一次响，
别每秒 tick 都响），并**同时震动**——静音键拨上时震动是唯一的信号。

三个出口都要 `QingScheduler.cancel()`。

> **⛔ 权限必须真的接上，否则 §6.3.1 定案二等于没做。**
>
> 初版规格写了 `QingScheduler.requestAuthorization()`、写了「拒绝则降级为仅前台响
> 并当场告知」，却**没有一处调用它**——`schedule()` 会照排通知，而 iOS 对没授权的
> App 静默丢弃。结果是锁屏引磬**一次都不会响**，用户还以为设了倒计时就有人替他守着。
> Task 16.5 的实现者发现了这一点并停下报告，处置正确。
>
> **接线点是「用户第一次真的设倒计时」那一刻，不是按「开始」那一刻。**
> 系统弹窗是模态的，放在按开始时会把 `schedule(after:)` 的起点往后推几秒；
> 放在选档位时，等他按开始权限早已尘埃落定。选「不限时」**不问**——他没设倒计时。
>
> ```swift
...
> ```
>
> 告知用既有的 `.alert` 写法。**文案要把「账不受影响」说在前面**——用户听到
> 「不会响」第一反应是怕漏记：
>
> ```swift
...
Run: `make test`
Expected: 全绿，总数增至 292

- [ ] **Step 10: 提交**

```bash
git add Sources/Core/WAVWriter.swift Sources/Core/QingSound.swift \
        Sources/Core/QingScheduler.swift Sources/Core/ClickSound.swift \
        Sources/Core/AppSettings.swift Sources/Features/Timer/TimerViewModel.swift \
        Sources/Features/Timer/TimerView.swift \
        Tests/QingSoundTests.swift Tests/TimerViewModelTests.swift
git commit -m "$(cat <<'EOF'
feat: 倒计时与引磬

§6.3.1 四问定案。到零不入账，走时钉死在到零那一刻，等用户按「记上」——
自动入账等于人不在场也记，直接顶「一声都不能多」。他 40 分钟后才回来，
记的仍是设的那 30 分钟。

最危险的一处是封顶会毁掉四小时闸门的证据：elapsed 封顶后，坐了 30 分钟和
早课忘按结束、晚上才打开，读数一模一样。所以另留一个不封顶的 rawElapsed
专供闸门，封顶只管显示与入账。

到零之后停心跳。照打的话草稿 updatedAt 会一直往前走，App 崩了之后
DraftRecovery 按 updatedAt − startedAt 估，会绕过封顶从后门把「多」放进来。

后台走本地通知，权限延后到第一次真的设倒计时才要，拒绝则降级为仅前台响
并当场告知。不用后台音频——它会打断用户正在放的念佛机。到零那刻 App 若已
被杀则两条路都不响，所以引磬永远只是「尽量响」，账绝不依赖它。

三个出口都撤通知：设了 30 分钟、10 分钟就下坐，20 分钟后突然响一声是必现骚扰。
请求用固定标识符，否则回前台五次就排了五声。

引磬是合成的不是录音：法布施 + GPL，录来的采样会带着版权来源问题。
WAVWriter 抽出来让两个音源共用一份头部——各写各的，迟早有一份填错某个数值
字段，而错了的那份不会崩，只会让声音闷掉。

档位没有「一炷香」：它的长度因寺院因香品而异，没有 108 那样的客观定数。
给自己挑的分钟数贴传统标签是编传统，跟编数是同一种罪。
...
- [ ] **Step 11: 真机手工验证（模拟器照不出来）**

1. **背景音乐不中断**：放一段音频，进计时器设倒计时并到零——引磬响，音乐不断
2. **锁屏到零**：设 1 分钟，锁屏等——引磬应当从通知响出来
3. **静音键**：拨到静音，重复上面两项——都不该出声，但**应当震动**
4. **提前下坐**：设 5 分钟，1 分钟就按结束——**五分钟后不许再响**
5. **回前台不重排**：设 5 分钟，反复切后台切回来五次——到零只响**一声**
6. 拒绝通知权限后再设倒计时——应当当场告诉用户「锁屏时不会响」，
   **且只告诉一次**（`qingLockScreenNoticed` 落盘）；杀掉 App 重进再设一次不该再弹

- [x] **Step 12: 审查收口（2026-07-31 完成）**

实现者交付 `071cbf9`，292 全绿、13 个代码块逐字节零偏差、`ClickSoundTests` 删除行数 0。
它自拟 4 个变异全部与预测一致（含我钦点的两个：闸门读封顶值 🔴、去掉 `guard !hasReachedZero` 🔴）。

**它报了两处，一处是我错、一处是我漏：**

**① `Int8(bitPattern:)` 那段我担心的类型问题不存在。** 我在派活时说「没编译过，很可能碰到问题」，
实测原样即通过。类型链成立：`data.dropFirst(44)` 的 `SubSequence` 就是 `Data` → 下标取 `UInt8`
→ `Int8(bitPattern:)` 合法 → `Int32(Int8)` 符号扩展。**我预警错了，记下来。**

**② 通知权限有函数、有文档、有承诺，就是没人调。** `requestAuthorization()` 是死代码，
Step 11 只把它列进手工验证清单，17/18/19 都没接。`schedule()` 照排通知而 iOS 静默丢弃
——**§6.3.1 定案二等于没落地**。实现者拒绝凭空造一段无测试兜底的权限 UI 并停下报告，处置正确。
已由我接线（见 Step 8 的 ⛔ 段）。

**我自己另查出一处，两人都没看见：**

**闸门抛出去的数被封顶污染了。** `finish()` 的闸门读 `rawElapsed`（这一处做对了），
但 `throw implausibleDuration(seconds: elapsed)` 带的是封顶值。这个数唯一的去处是纠正表那句
「计时器上是 X，多半是忘了按结束」——倒计时模式下就成了「计时器上是 **30 分钟**，多半是忘了按结束」。
30 分钟是个再正常不过的一坐，用户读完只会顺手拨 30 分钟，**记上一个从未发生的 30 分钟**。

原测试写的是 `#expect(throws: TimerViewModelError.self)`，「响了」和「响了但报的数是编的」分不出来。

```
RED   ✘ expected ".implausibleDuration(seconds: 28800)" but ".implausibleDuration(seconds: 1800)" was thrown
GREEN ✔ Test run with 292 tests in 0 suites passed
```

**还有一处覆盖空洞**：`qingEnabled` 是本任务新加的用户可见设置，**全仓零条测试碰它**。
而 `AppSettingsTests` 里那条测试自己的注释正写着为什么必须验。已补，并把名不副实的
`三个设置都能跨实例持久`（现在有五项）改名。

**审查后的变异实测（四条，全中）**：

| 变异 | 预测 | 实测 |
|---|---|---|
| 闸门 payload 退回 `elapsed` | 🔴 | 🔴 `expected 28800 but 1800 was thrown` |
| `qingEnabled` setter 去掉 `defaults.set` | 🔴 | 🔴 `(重启后.qingEnabled → true) == (引磬 → false)` |
| `_qingLockScreenNoticed` 读 `Key.qing` | 🔴 | 🔴 **两轮都红**（单向测试只会红一条） |
| 档位行去掉 `ensureQingAuthorization()` | 🟢 | 🟢 **292 全绿** |

第三条证明「两个方向都验」的写法真买到了东西。第四条是**第四次**坐实视图接线零覆盖
——权限接线整个删掉，全卷一条不红，只能靠 Step 11 第 6 项手工验。

- [ ] **Step 13: 收工**

```bash
git add Sources/Core/AppSettings.swift Sources/Features/Timer/TimerView.swift \
        Sources/Features/Timer/TimerViewModel.swift \
        Tests/AppSettingsTests.swift Tests/TimerViewModelTests.swift \
        docs/superpowers/plans/2026-07-28-counting.md
```
...
并在 `canSubmit(at:timeZone:)` **之前**插入：
...
Run: `make test`
Expected: 全绿，总数增至 298

- [ ] **Step 5: 写视图的失败测试**

> **⚠️ 这一批 2026-07-31 按纪律第 9 条复审过，改了三处。别照初版写。**
>
> | 初版 | 病 | 处置 |
> |---|---|---|
> | `流水一行的说法带来源与时刻` | **名字比断言宽**（⑭）：断言只碰 `amountText` 和 `isRevocation`，**来源和时刻一个都没验** | 改名 `数量的说法带正负号而撤销那笔认得出来` |
> | `各来源都有中文说法` 只验 `!isEmpty` | **会为错误的原因通过**（⑪）：`default: "其他"` 的兜底、以及把两个分支的字串写反，两种都非空 | 逐个钉死字面量 + 验互不重复 |
> | `timeText` **一条测试都没有** | 它是那个被删掉的名字承诺过的「时刻」，而且是全卷唯一一处「历史流水按存下来的时区显示」 | 新增 `流水的时刻按记那会儿的时区说而不是按此刻手机的时区` |
>
> 第三条是这一批里唯一真正危险的：`timeText` 里那句
> `TimeZone(secondsFromGMT: s.tzOffsetMinutes * 60)` 退回 `.current`，
> 在北京记的一坐飞到美国再看就整体挪 15 小时。**账一秒没错，说谎的是屏幕。**
> 期望值必须写死字面量——拿 `.current` 去算期望值就成了**同一把歪尺子**（⑬）。
...
6. **选中打坐（计时类）后这一页还转得动**——见 Step 11 ①，那处从前会锁死重绘
7. 进「记入以往的累计」敲个数、按取消，退回来**数量框是空的**——见 Step 11 ②

- [ ] **Step 11: 审查收口（2026-07-31）**

交付 `67d2d0a`（304 全绿、6 个代码块逐字节零偏差）。审出三处，都在视图里，
都没有任何测试能覆盖——这是本卷第五次撞上「视图层接线零覆盖」。

**① `canSubmit` 在 `body` 求值途中往 `vm.amount` 里写。**
...
它由 `.disabled(!canSubmit)` 调用，而 `vm.canSubmit()` 经 `validate` 又读
`vm.amount`——**同一趟 `body` 里对一个 `@Observable` 属性既读又写**。
`@Observable` 生成的 setter 不比较新旧值，写进去就发通知，于是
写 → 失效 → 重绘 → 再写。计时类功课（打坐）一被选中，补记页就卡在重绘里。

改法：转盘的值由 `body` 上的 `.onChange(of: hours/minutes)` 推进去，
与三行之上 `pickedDate` 的写法一致；`canSubmit` 退回纯读；`submit()` 里那句
重复的赋值换成同一个 `syncDial()`，好让「校验的数」与「记的数」只有一个来源。

**② 迁移表单与补记表单共用同一个 `vm.amount`，而取消时没人擦。**

在迁移表里敲 290000，想想不对按「取消」，退回补记页——数量框里就是 290000，
顺手一点「记上」，**今天凭空多出 29 万声**。这是全卷方向最坏、量级最大的
一处「多」，而它只是因为两张表共用一个数。改法：`MigrationSheet` 进出各擦一次
（`.onAppear` 与「取消」按钮）。

**③ 换功课时上一门课的数还留在框里。** 打坐转盘拨到 30 分（`amount` = 1800），
改选念佛，数量框里赫然是 1800。**预填等于替用户编一个数**——`TimerView` 的
纠正表早就写过这句话。改法：`.onChange(of: vm.selectedItem?.id)` 里 `resetInput()`。

**另补一条测试**（交付时实现者自己报的空洞）：`amountText` 的 `.check` 分支
（「已完成」/「取消」）零覆盖。那一行三元判断写反，修正页就把撤销那笔说成
「已完成」、把原记录说成「取消」，**一天的实情被整个说反，而用户正是到这儿
来核对的**。与「时刻按存下来的时区说」同族：账没错，说谎的是屏幕。
...
**实现者唯一一处偏离经核实完全必要**：`migrationNote` 加 `nonisolated`。
去掉它复跑，编译错误原文一致：
`main actor-isolated static property 'migrationNote' can not be referenced from a nonisolated context`。
`EntryRow` 是 nonisolated 的 enum，而 `ManualEntryViewModel` 是 `@MainActor`。计划遗漏。

变异证据（都串行跑、数字对得上）：

| 变异 | 预期 | 实测 |
|---|---|---|
| `timeText` 的 `TimeZone(secondsFromGMT:)` → `.current` | 红 | 红 `"23:30" == "14:30"` |
| `sourceText` 的 `.counter`/`.timer` 对调 | 红 | 红 `"计时器" == "计数器"` |
| `isMigration` 的 `==` → `!=` | 红 | 红 |
| `amountText` 的 `.check` 三元判断写反 | 红 | 红（两条断言各红一次，305） |
| `amountText` 的 `"取消"` → `"撤消"` | **绿**（旧空洞） | **绿 305**——补测前的真实状态 |
| 去掉 `migrationNote` 的 `nonisolated` | 编译失败 | 编译失败 |

- [ ] **Step 12: 收工**

Expected: 全绿，总数增至 305
...
> **⚠️ 这一批 2026-07-31 按纪律第 9 条复审过，改了四处。别照初版写。**
>
> | 初版 | 病 | 处置 |
> |---|---|---|
> | `计时类与勾选类不需要量词` | **名字比断言宽**（⑭）：只走了 `.duration`，实现写成 `if measureType == .duration` 时勾选类漏网 | 补 `.check` 一段 |
> | `图标与颜色都从预置里挑` 的 `#expect(!vm.iconName.isEmpty)` | **会为错误的原因通过**（⑪）：SF Symbol 名字写错不报错、只渲染成空白，而「非空」照样成立 | 改成 `iconChoices.contains(…)` |
> | **切换量法不清目标值** | 真 bug，见下 | `didSet` 里补 `dailyGoal = nil`，并新增 `换量法时目标值也一并清掉` |
> | `init` 里「`measureType` 必须先赋：它的 didSet 会清空量词」 | **注释说的事实不成立**（⑧/⑩）：`didSet` 在类自己的 `init` 里根本不触发（已用 swiftc 实证） | 改写注释说真话，并把 `_dailyGoal` 挪到 `measureType` 之后 |
>
> 第三条是真 bug：`dailyGoal` **计数类存遍数、计时类存秒**，两套单位之间没有
> 任何有意义的换算。计数类下设目标 30（遍），改成计时类，`dailyGoal` 仍是 30，
> 而计时类按秒读——**「打坐 30 秒」就算圆满**。`goalDisplay` 读回来是 `30 / 60 = 0`，
> 界面上什么都不显示，用户按保存就把这个假目标落了库。
...
Run: `make test`
Expected: 全绿，总数增至 324

- [ ] **Step 5: 变异验证「改目标不改历史圆满」这条保障会咬人**

> **⚠️ 2026-07-31 更正：这里从前写的是「去掉 `PracticeItemStore.update` 里那道守卫」，
> 而那道守卫根本不存在。**
>
> 「改今天的目标不会把 7 月 1 日的圆满收回去」不是靠哪一行 `if` 守着的，
> 是靠 `update` **什么都不做**撑着的——它只写 `PracticeItem`，从不碰 `DaySnapshot.goals`。
> 所以这份保障看起来像个疏忽，很容易被后来人好心「补上」。变异因此要**反着做**：
> 往 `update` 里**加进**把 `dailyGoal` 传播到快照的代码，看安全网会不会响。
>
> 已在 `PracticeItemStore.update` 里留了一段注释挡住这条路。
...
> `模板色板每支在两个背景上都够得着图形对比度` 就要读 `colorChoices`，留到这里加，Task 13 编译不过。
...
6. **计量切到「打勾」时，整个「每日目标」区不见了**——见 Step 14

- [ ] **Step 14: 审查收口（2026-07-31）**

交付 `b8faae5`（332 全绿，6 个代码块中 5 个逐字节零偏差）。

**唯一一处偏离经复现核实完全必要**：`改目标不改历史圆满` 里
`ledger.plan(for: 20260701, activeItems: [item])` 缺 `dayStartHour:` 与
`timeZone:`——这两个参数**没有默认值**。计划遗漏。

**实现者还纠正了 Step 5 的变异描述**（那道「守卫」不存在），已在上面更正。

**我审出一处，方向是圆满被夺走**：

初版 `goalChips` 把计数类那套档位（`[nil, 108, 1080]`）**原样给了打勾类**，
于是「每日目标」连同 108、1080 两个档明晃晃摆在早晚课的表单上。
而 `LedgerMath.isFulfilled` 算的是 `total >= goal`，打勾类一天最多记 1——
用户随手点一个 108，从此**天天打勾、天天不圆满**，而且看不出是为什么：
他不会想到是那个自己点过的数。

圆满是这个 App 唯一一处替用户下的判断，不许它被一次随手点击永久夺走。改法：
...
`dailyGoal` 的 setter 加 `measureType != .check`，`init` 改走 setter
（顺手洗掉同步来的「打勾类带着目标」这种脏组合），视图 `if vm.measureType != .check` 整节隐藏。
**两侧都挡，不只靠视图**——视图层零覆盖，只挡在那儿等于没挡。
...
| 变异 | 预期 | 实测 |
|---|---|---|
| 往 `update` 加进目标传播（见 Step 5 更正） | 红 | 红 3 条 |
| `didSet` 去掉 `dailyGoal = nil` | 红 | 红 `换量法时目标值也一并清掉` |
| `!= .count` → `== .duration` | 红 | 红 `计时类与勾选类不需要量词` 的 `.check` 段 |
| 同上 + 测试删回只验 `.duration` | **绿**（旧网眼）| **绿 332**——成对反证 |
| `goalDisplay` getter 除以 120 | 红 | 红 2 条 |
| `goalLabel` 里「分」→「minutes」 | **绿**（视图层空洞）| **绿 332** |
| `goalChips` 的 `.check` 改回 `goalChips` | 红 | 红 1 条 |
| `dailyGoal` setter 去掉 `!= .check` | 红 | 红 2 条 |

最后两条分别验两侧守卫，各自独立咬人。

- [ ] **Step 15: 收工**

Expected: 全绿，总数增至 333
...
> **⚠️ 2026-07-31 派活前规格复审（纪律第 9 条）——已改两处，照下面写的做**
>
> **① `runAtLaunch(now: Date = Date())` 的 `now` 是死参数**，函数体一处未用，
> 十二条测试也全是 `runAtLaunch()` 无参调用。已删。留着它会让人以为
> 「启动清算」跟当下时刻有关，然后照着这个错觉去改。
>
> **② `accept` 的 `dayStartHour: Int = 0` 是个静默陷阱**，而 `RecoveryCoordinator`
> 手里就攥着 `env.settings`。`AppSettings.dayStartHour` 的注释早就写明这一族默认值
> 「漏传一处，那一处就静默退回 0 点，而没有任何测试会红」。
> 这条路上漏传的代价最大：**永久写进账本、用户看不见**。已改成直接从 `env.settings` 读，
> 并新增一条 `恢复入账认用户设的一日起始` 钉住（Step 1 末尾）。
> `timeZone` 仍留作参数——它没有对应的用户设置，且测试必须能注入固定时区。
>
> 连带改了 Step 8 的 `RootView` 调用点（`try recovery.accept(first)`）。
...
- [~] **Step 12: 端到端手工验证（2026-07-31 跑了能自动化的那半；余下需人点击）**

**已跑（自动）**：`xcodegen` → build for simulator → install → launch → 截图 → 采样像素。
浅色深色各一轮，**无崩溃日志、空态渲染正确**。

**跑出两条，一条修了一条记账：**

**① 修了：色板底色一像素都没画出来过**（`0cd7417`）。
`ThemedBackground` 那句 `.background(theme.background.ignoresSafeArea())` 挂在
`NavigationStack` 外面，被它自己那层不透明系统底整个盖住 —— **死代码**。
截图采样：浅色实测 `#FFFFFF`（色板写 `#FAF7F0`）、深色实测 `#000000`（色板写 `#17140F`）。
要紧的不是好不好看，是 **`Palette.audited` 那 11 条对比度断言审的是一个用户从没见过的底色**。
拆出 `.pageBackground()` 挂进栈内（根页 + `navigationDestination` 两处），
修后采样与色板逐字节吻合。

**⛔ 视图层零覆盖的第八次实测，也是最狠的一次**：整块底色一像素没画出来，
**356 条测试改前改后全绿**。前七次还得人为造个变异才照得出洞，
**这一次 production 代码本身就是那个变异，而它绿着。**

**② 记账不改：全新安装同屏显示「今日无课」+「还没有定课」。**
两句都由 `isRestDay`（`requiredCount == 0`）驱动，各自都为真，但并排读起来
把「你还没设置」框成了「今天恰好没排课」。真要分开得有 `ScheduleRule`，
那是第 6 卷；首次启动的呈现是 §6.10、圆满三态的呈现是 §6.8，**两者都已划给第 5 卷**。
不动，只记在这里。**不影响账本：`isRestDay` 眼下还管着 `TodayView` 的「空态 / 功课清单」分支，
两句文案只是那个分支露在外面的样子——**它不进任何一次写入**。**

**读代码另核了两条不点也验得了的：**

- **第 6 条（弹窗必须带日期那一行）**：`RootView.body` 的恢复弹窗里是
  `Text(recovery.pending.first.map { Self.recoveryMessage($0) } ?? "")`，
  而 `recoveryMessage` 的日期行有测试，且「`kind` 写死」的变异补测后会红。接线正确。
- **第 11 条（两份草稿一份一份问）**：`isPresented: .constant(!recovery.pending.isEmpty)`
  配 `pending.first`，答完一份 `pending.removeAll` 掉它，弹窗条件仍为真、
  内容换成下一份。**结构上保证**，不是靠调用顺序。
  且 `accept` 经 `commit` 的 `sessionID` 查重，重复点「记上」不会写两笔；
  状态清理排在会抛的那步之后（纪律 6 ✓）。

**余下必须人来点的**（模拟器 + 手指，`simctl` 没有点击能力，本卷不引入 XCUITest）：
...
6. 重开 App → 应弹窗「上次「念佛」计数到 10 声时应用退出了，还没记上。
   记上会算在 X月X日 HH:mm，也就是当时那一天。」——**日期那一行必须在**，见 Step 13
7. 点「记上」→ 今日页显示 30 声
8. 再杀一次、再数几下、这次点「不记了」→ 今日页数字不变
9. 补记页给昨天补 500 声 → 「这一天的记录」多一行 → 点「撤销」→ 多一行减号行，原行还在
10. 定课管理里把「念佛」收起 → 今日页当天仍显示且标「已归档」，第二天起消失
11. **留两份草稿再重开**（分别杀在念佛和打坐里）：弹窗应当一份一份问，
    答完第一份第二份接着弹，两份都答完才放行
12. **顺手看一眼 sheet 的底色**（批量档位 / 模板选择 / 迁移 / 超时长转盘 / 自定义分钟）：
    它们从来没写过 `.background(theme.background)`，眼下是系统底。暖底页面上开出
    一张纯白 sheet 好不好看，是取向问题不是 bug，**由你定**：要跟着暖色的话，
    给这五处 sheet 内容各加一个 `.pageBackground()` 即可（`Timer:115` `Timer:176`
    `ManualEntry:88` `Counter:52` `ItemList:73`）

- [ ] **Step 13: 审查收口（2026-07-31）**

交付 `509146a`（350 全绿）。**5 个代码块逐字节零偏差**（`diff-plan-vs-disk.py` 全部命中），
两个检查点 346 / 350 都真跑出来，`git show --stat` 的文件清单与 Step 11 完全一致。
四个变异全中，含计划点名的那一个。派活前的规格复审（删 `runAtLaunch(now:)` 死参数、
`accept` 直接读 `env.settings.dayStartHour`）与最终代码完全吻合。

**实现者诚实报了一个空洞**：`recoveryMessage` 的「计时/计数」分支零覆盖，
并且**预测它的变异会绿**——预测正确（见下）。

**我审出一处，方向是「多」，而且概率不低**：

`accept` 把这笔记在**功课发生的那天**（`at: draft.updatedAt`）。那条守卫是对的，
本任务专门为它立了两条测试。但它带出一个新问题：

> 隔夜、隔天才重开 App 的用户，按完「记上」回到今日页会发现**什么都没变**——
> 那 108 声落在昨天了。他刚认下的一笔像是凭空消失，于是照着记忆再手动补记一遍。
> 今天就多出 108 声，而昨天那 108 声他还不知道。

**凡是过夜的草稿全走这条路**，这不是边角情形。而弹窗原本只说
「上次「念佛」计数到 108 声时应用退出了」——「上次」听起来就是刚才。

**「一声都不能多」在这里不是被一行代码破掉的，是被一句没说出口的话破掉的。**

改法：`PendingRecovery` 补 `endedAt`（即 `draft.updatedAt`，`accept` 拿来算落哪天的
就是它），弹窗把日期和时刻说出来。**弹窗承诺的和 `accept` 做的必须是同一件事**——
所以取的是同一个字段，不是另算一个。

措辞只陈述事实、不判断是不是今天：同一天崩溃再重开也是常事，那时说「不算今天」
就是撒谎。写出日期，用户自己认得出。
...
`timeZone` 留作参数供测试注入——本机是 PDT，拿 `.current` 断言就是拿实现那把尺子
量实现（纪律 ⑬）。红绿都跑过：

新增两条测试（`Tests/RootViewTests.swift` 追加）：
...
```
RED   error: extra argument 'timeZone' in call        （编译期红）
GREEN ✔ Test run with 352 tests passed
```

第二条补的是实现者报的那个空洞，两个方向都验且**互斥**断言——只写 `contains("计数")`
会为错误的原因通过（⑪），因为把两个分支写反时两句话都非空、都含「计」。

变异证据（都串行跑、数字对得上）：

| # | 变异 | 预期 | 实测 |
|---|---|---|---|
| M1 | `reconcilePendingDrafts()` → `pendingDrafts()`（计划点名）| 红 | 红 `已入账的草稿被静默丢弃不弹窗` |
| M2 | `accept` 的 `at: draft.updatedAt` → `at: Date()` | 红 | 红 `隔夜才点头` + `恢复入账认用户设的一日起始` |
| M3 | `dayStartHour: env.settings.dayStartHour` → `0` | 红 | 红 `恢复入账认用户设的一日起始` 两条断言 |
| M4 | `guard DraftRecovery.matches(…)` → `guard true` | 红 | 红 `量法改过的草稿启动时静默作废不弹窗` |
| M5 | `endedAt: draft.updatedAt` → `draft.startedAt` | 红 | 红 `恢复弹窗说清了这笔会算在哪天` 的 `21:40` |
| M6 | `kind` 写死 `"计数"` | 红 | 红 `恢复弹窗分得清计数与计时` |
| M6′ | **同一变异跑在补测之前** | **绿**（旧网眼）| **绿 350**——成对反证 |
| M7 | `isReady` 去掉 `&& rc.pending.isEmpty` | 红 | 红 `还有待裁决的草稿时也不放行` |

M6 / M6′ 是**第七次**实测确认「视图层接线零覆盖」，也是第一次当场把洞补上：
`recoveryMessage` 是可测的纯函数，本来就该被钉住。M5 之所以能咬人，是因为断言写到了
**分**——只验「7月28日」的话，`startedAt`（21:30）和 `updatedAt`（21:40）同一天，
变异会大摇大摆走过去。

**已知空洞（诚实记下，不假装覆盖）**：

- `accept` / `discard` 的 `guard` 失败路径（`drafts[item.id]` 为 nil）零覆盖。
  要「弹窗显示了但字典里找不到」才触达，理论上够不着。
- `RootView` 的两个 `.alert` 若同时为真（`accept` 抛错时 `pending` 不变、
  `failure` 又被设上），`isPresented` 都是 `.constant`，恢复弹窗永远不消失，
  错误弹窗排不上队 —— 用户会卡在一个点不掉的弹窗里。要 SwiftData 持续写失败才踩得到，
  那时整个 App 本就废了。**没在本卷修**：加重试要么绕过清算（放用户进计时器接旧草稿），
  要么就是另一套错误恢复设计，两者都超出本任务。记在这里，第 3 卷接同步时一并想。

- [ ] **Step 14: 收工**

Expected: 全绿，总数增至 352
...
| **把 `saveOrRollback` 铺到三个 store 的全部九个写入口** | 单独立项，第 3 卷开同步前 | 见下 |

### 那九个写入口（全卷终审 2026-07-31 找到的唯一一处结构脆弱点）

`DayLedger` 的两个写入口有 `saveOrRollback`，`DraftStore.commit` 有 `rollback()`。
**其余九处是裸 `try context.save()`**：

- `DraftStore`：`begin:51`、`update:62`、`touch:69`、`discard:115`
- `PracticeItemStore`：`create:102`、`update:182`、`archive:188`、`unarchive:204`、`reorder:213`

三个 store **共用同一个 `ModelContext`**（`AppEnvironment.swift:32`），而
`DayLedger.saveOrRollback` 的注释里已经实测写明：SDK 26.5 下 `save()` 抛错时
store 会一致回滚，但 **context 的 pending 改动不会被清掉**，会被下一次
**无关的** save 顺手提交。`DraftStoreTests` 的 `提交失败后不留残念给下一次save`
把这条语义钉住了。

**为什么本卷不动它**：

1. 残留够不到账本的数。九处残留的全是 `PracticeItem` 的配置字段
   （name / measureType / unit / dailyGoal / isArchived / activatedAt / sortOrder）
   或草稿字段，不含任何流水的 `amount` / `dayKey`。`measureType` 的改写发生在
   `hasAnyHistory` 守卫**之后**（`update:162`），迟落盘也不会去重新解释一笔
   历史流水；历史圆满读的是冻结快照。
2. **但终审说的「够不到账本」说轻了一处**：`activatedAt` 会经
   `DayLedger.appendLateArrivals` 流进 `DaySnapshot`，也就是圆满的分母。
   `unarchive` save 失败、界面说了「失败」、之后被无关 save 顺手落盘 →
   那门课悄悄回到今日页、今天的圆满分母多一门。
   **不是静默的数字错**（用户在列表里看得见这门课回来了），但确实够得到圆满。
3. 真要改就得**九处一起改**，只补 `PracticeItemStore` 会让三个 store 里两个设防、
   一个不设防，比现在更难读。
4. **写不出真正的红绿**：`save()` 失败没法在测试里诱发（除非造唯一性冲突），
   而没有红绿的加固只是看起来像加固。要么先把诱发失败的手段做出来，要么不做。

第 3 卷开 CloudKit 之后写入冲突变多，那时一并做，并补上诱发失败的测试手段。

### 计划外补做：§5.7 跨设备重复撤销双扣（2026-07-31）

**这是第 3 卷的内容，提前做进了第 2 卷。写清为什么，免得后来人以为是范围失控。**

三条理由，按分量排：

1. **防线要在它要防的事发生之前就位。** 第 3 卷第一天拨开 `cloudKitDatabase`
   开关，双扣就可能发生；等那时再补，中间那段窗口里产生的错账没人补得回来——
   用户看到的只是数字变小了，没有任何提示。
2. **它现在就能测，而且测得完整。** 去重是纯读侧函数，喂两条 note 相同、
   UUID 不同的 `.adjustment` 就复现，不需要真容器、不需要两台设备。
   等到第 3 卷再做，反而要跟真同步的不确定性缠在一起。
3. **它不动写侧一个字节。** `revoke` 原封不动，`sessions(on:itemID:)` 多过一道
   纯函数。今天所有测试都不该受影响——实测 352 → 356，原有 352 条一条没动。

   ⚠️ **「不动写侧」说的是 §5.7 自己的范围，不是「本卷已无剩余数据完整性闸门」。**
   那九个裸 `context.save()` 仍然挂着：三个 store 共用一个 `ModelContext`，
   save 抛错后 pending 改动不清空，会被下一次无关的 save 顺手提交；
   `activatedAt` 还经 `appendLateArrivals` 流进 `DaySnapshot`，也就是圆满的分母。
   不在本卷改的四条理由见「本卷刻意不做」，**头一条是它眼下写不出真正的红绿**
   （SwiftData 里诱发 `context.save()` 抛错没有干净手段：实测删掉草稿再 `update`，
   `save()` 照样成功）。第 3 卷接 CloudKit 时连诱发失败的测试手段一并做。

**做的时候顺带定案了 design-spec §5.7 的三个待答**（原本与 §5.6 并列为第 3 卷
开工闸门，现已全部答完）：

| 问 | 答 | 依据 |
|---|---|---|
| 读侧还是写侧 | **读侧，写侧根本不可用** | CloudKit 用不了 `@Attribute(.unique)`（§5.2 自己记着）。没有唯一约束，两台设备各写一条就永远是两条；「事后清理」也不成立，只要还有第三台设备可能上线补一条 |
| 存量怎么办 | **什么都不用做** | 读侧去重是幂等的，任何时刻算出来都对 |
| 「撤销那次撤销」怎么算 | **这条路不存在** | `ManualEntryView:160` 不给撤销行配撤销按钮；`revoke` 是全仓库唯一写 `.adjustment` 的地方且永远写 `-session.amount`，库里不存在正数 `.adjustment` |

⚠️ **§5.7 初稿写的「现在靠 `.adjustment` 里的正数实现」是编的**——同 §5.6 初稿
那次（纪律 ⑩）。写下「现在靠 X 实现」之前先 grep，找不到就老实说这是新定的。

**变异实测（串行，基线 `df44aeb`）**

| 变异 | 预测 | 实测 |
|---|---|---|
| `dedupedRevocations` 直接 `return sessions` | 🔴 | 🔴 4 issue，只红 `两台设备各撤一次同一笔只扣一次` + `重复的撤销笔在流水里也只出现一次` |
| 分组键换成 `amount` | 🔴 | 🔴 1 issue，只红 `撤不同的两笔各扣各的` |
| 去掉 `note.hasPrefix("revoke:")` 只按 note 分组 | 🔴 | 🔴 2 issue，只红 `不带撤销标记的调整笔不参与去重` |
| 排序去掉 `id.uuidString` 次键 | 🟢 | 🟢 **356 全绿** |

**第三个变异是本次最值钱的一段，因为它先绿后红：**

那个 `revoke:` 前缀条件今天是**为将来承重**的——`revoke` 是眼下唯一写
`.adjustment` 的地方，note 全带这个前缀，所以按「note 相同」分组眼下完全等价。
我写了守门测试去钉它，第一版用 `note: nil` 造两笔调整，**变异跑出 356 全绿**：
`guard let note = s.note` 在两个版本里都提前 `continue`，测试根本走不到被变异的
那一行。**纪律 ⑪ 的教科书形状——它为错误的原因通过。** 改成两笔备注都写
「年度盘账」（非空、不带前缀）之后，同一个变异当场红 2 条。

| 守门测试写法 | 同一个变异 |
|---|---|
| `note: nil` | 356 全绿 |
| `note: "年度盘账"` | 红 2 条，且只红这一条 |

**第四个变异（次键）绿是已知空洞，不补。** 它只决定「重复的两条留下哪一条」，
而重复两条 `amount` 必然相同、`note` 必然相同，总数与流水条数都不受影响；
唯一差别是各设备列表里那一行的 `id` 可能不同，用户看不见。要钉住它得断言
「保留的是 id 字典序小的那条」，那是把实现抄进断言，买不到东西。
| Live Activity / 灵动岛 | **永不** | §2.2 明确不做 |
| Apple Watch | 1.1 | §8.6。append-only 让它不需要任何新的冲突处理，但表冠交互是独立一摊 |
| 音量键计数、蓝牙响指器、共修社交、任何服务器 | **永不** | 已定决策 |
| 音效 / 震动的开关界面 | 第 5 卷 | §6.2 要求两者「各自可独立开关」，`AppSettings.soundEnabled` / `hapticEnabled` 本卷已就位、默认全开、由 Task 16 的 `CounterView` 读取（`Feedback.shared.tick(sound:haptic:)`），但**全卷没有一个 `Toggle`**——能力有了，用户够不着。与下面一条同批，一并在设置页落地 |
| 一日起始时间的设置界面 | 第 5 卷 | `AppSettings.dayStartHour` 本卷已就位，Task 14 起各视图也会把它传给 store 和 ViewModel，只是没有界面去改它，值恒为 0。**接线时当心**：`DraftStore` / `DayLedger` 和三个 ViewModel 的 `dayStartHour` 都带着 `= 0` 的默认值，漏传一处就静默退回 0 点，而没有任何测试会红——每条测试都自己显式传 `dayStartHour:`。第 5 卷值不再恒为 0 之后，这批默认值应当改成必填 |
| 定课的删除（硬删） | **永不** | 只归档。硬删让流水变孤儿，孤儿被 `$0.item?.id == itemID` 挡在所有按项查询之外，等于静默蒸发 |
| 设置走 `NSUbiquitousKeyValueStore` 同步（spec `:217`） | 第 3 卷 | 与 CloudKit 同一道 entitlement 门槛。本卷设置只落 `UserDefaults`，第 3 卷开同步时一并接。**接的时候必须注意**：别的设备推过来的 `dayStartHour` 不经过 setter，越界值会直接生效——守卫见 `盘上已存的越界一日起始要在装配时夹回来` |
| §7.5 四选主题（跟随系统／浅色／深色／按时间） | 第 5 卷 | 本卷 `Theme` 只接 `ColorScheme`，即「跟随系统」一档。四选是设置界面的事，而设置界面在第 5 卷 |
