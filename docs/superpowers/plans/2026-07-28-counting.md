...
@Test func 拖拽排序按传入顺序重排() throws {
    ...
    #expect([c.sortOrder, a.sortOrder, b.sortOrder] == [0, 1, 2])
}
...
Run: `make test`
Expected: 全绿，总数增至 146

- [ ] **Step 5: 变异验证排序真的被约束住了**

把 `reload` 末尾的 `rows = built.sorted { … }` 临时改成 `rows = built`，跑 `make test`。
Expected: `按用户拖拽的顺序显示而不是按uuid` 变红（若恰好没红，说明三个 UUID 的字典序碰巧与拖拽顺序一致——把测试里三个模板换成 `mantra` / `sutra` / `prostrate` 再试一次，直到确认它真的会红）。
**改回去**，再跑一次确认恢复全绿。
