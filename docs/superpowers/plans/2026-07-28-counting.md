...
Expected: **只有** `跨零点结束后屏幕上的今日不带昨天的数` 变红（`dayTotal` 得 5400 而非 1800）。
...
Expected: `结束时用的是进页面时那个时区` 变红（两个时区都退回 `.current`，落在同一天）；
`跨零点结束后屏幕上的今日不带昨天的数` 也会红。

**变异 5：`DraftStore.commit` 的 `at: now` 改成 `at: draft.startedAt`**（跨文件，记得改回）

「记在开始那天」是一条**真实的竞争实现**，光看名字还挺有道理。
Expected: `落哪天按结束那一刻算而不是开始那一刻`、
`跨零点结束后屏幕上的今日不带昨天的数` 变红。
