...
@Test func 缺的定课同步下来之后自己就好了() throws {
    ...
    #expect(vm.rows.contains { $0.name == "拜佛" && $0.state == .unfulfilled })
}
...
Run: `make test`
Expected: 全绿，总数增至 166

- [ ] **Step 5: 变异验证「中途不写账」真的被约束住**

把 `add(_:at:)` 临时改成在更新草稿之后再补一句
`try ledger.record(item: item, amount: delta, source: .counter, startedAt: now, at: now)`，跑 `make test`。
