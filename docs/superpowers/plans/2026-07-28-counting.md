...
private func makeManual() throws
    -> (ManualEntryViewModel, PracticeItemStore, DayLedger, PracticeItem) {
    let ctx = ModelContext(try ModelContainerFactory.inMemory())
    let ledger = DayLedger(context: ctx, deviceName: "iPhone·TEST")
    let items = PracticeItemStore(context: ctx)
    let item = try items.create(name: "念佛", measureType: .count, unit: "声",
                                dailyGoal: 1000, iconName: "circle.grid.3x3",
                                colorHex: Palette.Light.fulfilled)
    return (ManualEntryViewModel(ledger: ledger, items: items), items, ledger, item)
}
...
@Test func 补记到指定历史日期() throws {
    let (vm, _, ledger, item) = try makeManual()
...
@Test func 补记的写入时刻是真实的不是回拨的() throws {
    ...
    let (vm, _, _, item) = try makeManual()
    let 写入时刻 = 北京(7, 28, 21, 0)
    vm.selectedItem = item
    vm.selectedDayKey = 20260701
    vm.amount = 500
...
@Test func 补记不覆盖已有快照() throws {
    // §6.4：若已存在则沿用，**绝不覆盖**。
    let (vm, items, ledger, item) = try makeManual()
...
@Test func 不许补记到未来() throws {
    ...
    vm.amount = 100
    #expect(!vm.canSubmit(today: 20260728))
...
final class ManualEntryViewModel {
    ...
    func canSubmit(today: Int) -> Bool {
        selectedItem != nil && amount > 0 && !DayKey.isFuture(selectedDayKey, comparedTo: today)
    }
...
Run: `make test`
Expected: 全绿，总数增至 218

- [ ] **Step 9: 变异验证「不覆盖已有快照」真的被约束住**

把 `submit(note:at:dayStartHour:timeZone:)` 里那句
`_ = try ledger.plan(for: selectedDayKey, activeItems: try items.activeItems())` 临时注释掉，跑 `make test`。
Expected: `补记历史日期会补建快照` 变红（`补记不覆盖已有快照` 仍绿——因为那条测试自己先建了快照，这正好说明两条测试各咬各的）。
**改回去**，再跑一次确认恢复全绿。
...
struct ManualEntryView: View {
    ...
    private var canSubmit: Bool {
        ...
        return vm.canSubmit(today: today)
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("记上") { submit() }
                        .disabled(!vm.canSubmit(today: DayKey.today(dayStartHour: settings.dayStartHour)))
                }
            }
            .alert("出了点问题", isPresented: .constant(failure != nil)) {
                Button("知道了") { failure = nil }
