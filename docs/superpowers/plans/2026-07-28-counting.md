...
调整，诊断页无法解释。下限 0：账本允许为负（同步冲突的真实状态），
但「正在数的这一笔」不可能是负的。
...
/// 批量增加。§6.2：拨完一整串念珠一次加 108，或从实体计数器上誊抄。
...
struct CounterView: View {
    ...
    private func commit() {
        do { _ = try vm.finish(dayStartHour: settings.dayStartHour) }
        catch { failure = error.localizedDescription }
    }
}
```
