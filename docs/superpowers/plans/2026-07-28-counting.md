...
Run: `make test`
Expected: 全绿，总数增至 96。特别确认 `模型清单完整` 仍然通过（它断言同步 schema 恰好三个实体，草稿没混进去就该是绿的）
...
Run: `make test`
Expected: 全绿，总数增至 100。**第一卷原有的 DayLedger 测试一条都不许改**——`record` 的外部行为完全没变，若有变红说明重构错了。
