...
@Test func 撤销到零就停住不往负数走() throws {
    // 账本的当日求和**允许为负**：§3 的 adjustment 流水可正可负（design-spec `:149`、`:173`），
    // 而各设备的流水各自独立同步，一笔负的修正完全可能先于它要修正的那笔正数到达。
    // 但「正在数的这一笔」不可能是负的。
    let (vm, _, _, _) = try makeCounter()
    let now = 北京(7, 28, 9, 0)
    try vm.start(at: now, timeZone: 北京时间)
...
Run: `make test`
Expected: 全绿，总数增至 168

- [ ] **Step 5: 变异验证「中途不写账」真的被约束住**

把 `add(_:at:)` 临时改成在更新草稿之后再补一句写账。
**注意必须显式传 `timeZone`**：`DayLedger.record` 的 `timeZone` 默认是 `.current`，
而测试全部用北京时间构造 `Date`。开发机在西八区时 `北京(7,28,9,0)` 本地是 7/27，
dayKey 偏到 `20260727`，而断言查的是 `20260728`——变异写进去的流水根本不在被检查的那天，
**这条变异会假绿，什么都验不出来**（实测如此）。
...
Expected: `计数过程中一笔流水都不写` 与 `计数中撤销只减草稿不产生调整流水` 变红。
**改回去**，再跑一次确认恢复全绿。

> ⚠️ 这个坑对**整卷**都成立：任何在测试里比对 dayKey 的变异，
> 只要漏传 `timeZone`，都会因为 dayKey 偏移而假绿。Task 10–19 照此办理。
